...
(define (read-row row prev-label)
  (match row ((id rank category title summary location characters notes loc2
                  trigger)
              (if (zero? (string-length loc2))
                  ...
                  (let ((trig (if (zero? (string-length trigger))
                                  loc2
                                  trigger))
                        (text (string-join (list id title rank category summary
                    ...
                    (list`(("label" . ,label)
                           ...
                           ("location" . ,loc2)
                           ("trigger" . ,trig)
                           ("flags-required" . #(,@prev-label))
                           ("flags-exclusive" . #(,label))
                           ("lines" . #((("character" . "")
                                         ("text" . ,text)
                                         ("options" . #()))))
                           ))
